    ...
    stages {
        stage('SCM Checkout') {
            steps {
                ...
                git 'https://github.com/Sadhana752601/health-care.git'
       ...
       stage("Docker build"){
            steps {
				sh 'docker version'
				sh "docker build -t sadhana1996/health-care-eta-app:${BUILD_NUMBER} ."
				sh 'docker image list'
				sh "docker tag sadhana1996/health-care-eta-app:${BUILD_NUMBER} sadhana1996/health-care-eta-app:latest"
            }
        }
		...
		stage('Push2DockerHub') {
			...
			steps {
				sh "docker push sadhana1996/health-carep-eta-app:latest"
			}
		}
